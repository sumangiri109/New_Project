...
class AdminDashboardServices {
  ...
  Future<Map<String, dynamic>?> getLoanDetailsWithUser(String loanId) async {
    try {
      ...
      return {'loan': loan, 'user': user, 'kyc': kyc};
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> getUserDetailsComplete(String userId) async {
    try {
      ...
      return {'user': user, 'kyc': kyc, 'loans': loans};
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getDashboardStats() async {
    try {
      ...
      // Active loans
      final activeLoansSnapshot = await _db
          .collection("loans")
      ...
      // Pending loans
      final pendingLoansSnapshot = await _db
          .collection("loans")
      ...
      final verifiedKYCSnapshot = await _db
          .collection("kyc")
          .where("verified", isEqualTo: true)
          .get();
      final verifiedKYC = verifiedKYCSnapshot.docs.length;

      final pendingKYCSnapshot = await _db
          .collection("kyc")
      ...
      final newUsersThisMonth = await _db
          .collection("users")
          .where(
            "createdAt",
      ...
      // New loans this week
      final newLoansThisWeek = await _db
          .collection("loans")
          .where(
            "createdAt",
    ...
    try {
      ...
      for (var doc in recentLoans.docs) {
        ...
        activities.add({
          ...
          'description':
              '$userName applied for रु${loan.loanableAmount.toStringAsFixed(0)}',
          'status': loan.status,
